...
#[tauri::command]
async fn get_database_file(state: tauri::State<'_, AppState>) -> Result<Vec<u8>, String> {
    // 1. Get DB Path
    let db_path = {
...
#[tauri::command]
async fn overwrite_database_file(data: Vec<u8>, state: tauri::State<'_, AppState>) -> Result<(), String> {
    // 1. Get DB Path
    let db_path = {
        let service = state.lock().map_err(|e| e.to_string())?;
        service.db_path.clone()
    ...
    // 2. Write bytes (overwrite)
    // Note: On Windows, this might fail if the DB is locked by the app, 
    // but works fine on Android/Linux usually.
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            perform_sync,
            set_sync_server_url,
            get_database_file,      // New command
            overwrite_database_file // New command
        ])
